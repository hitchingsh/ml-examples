...
import sys
sys.path.append(".")
sys.path.append("./blenderbot2")
sys.path.append("../blenderbot2")
import json
import websocket
import threading
from parlai.core.params import ParlaiParser
from interactive_web import WEB_HTML, STYLE_SHEET, FONT_AWESOME
...
class BrowserHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path == '/interact':
            print("client.py received /interact call")
            ...
            print("client.py model_response text new_message=: ", new_message)
...
def _run_browser():
    ...
    print('This script has been modified by Hamilton to load interactive_web.py which sends to begin messages to chatbot so user does not have to')
...
if __name__ == "__main__":
    ...
    print("client.py Connecting to port: ", port)
    ...
    print("client.py Connected with ws: ", ws)
    ws.on_open = on_open
    ...
    while True:
        try:
            ...
            time.sleep(3)
            secs += 3 
            print(f"Failed to connect to blenderbot2, {secs} elapsed. Retrying...")
        except ValueError:
             print("Failed to run_forever, blenderbot2 is probably still starting up. Trying again...")
